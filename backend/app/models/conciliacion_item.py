...
class ConciliacionItem(Base):
    ...
    viaje_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("viajes.id"), nullable=True, index=True)
    tipo: Mapped[ItemTipo] = mapped_column(Enum(ItemTipo), nullable=False)
    estado: Mapped[ItemEstado] = mapped_column(Enum(ItemEstado), default=ItemEstado.PENDIENTE, index=True)
    ...
    placa: Mapped[str | None] = mapped_column(String(50), nullable=True, index=True)
